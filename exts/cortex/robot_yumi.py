...
class CortexGripper(Commander):
    ...
    def close_to_grasp(self, speed: Optional[float] = 0.2, force: Optional[float] = 1.0) -> None:
        ...
        self.send(CortexGripper.Command(0.023 / 2, speed=speed, force=force)) # change this value later.
...
class YumiGripper(CortexGripper):
    """ Yumi specific parallel gripper.

    Specifies the gripper joints, provides mappings from width to joints, and defines the yumi
    ...
    def __init__(self, articulation: Articulation):
        super().__init__(
            articulation_subset=ArticulationSubset(articulation, ["gripper_r_joint", "gripper_r_joint_m"]),
            opened_width=0.05,
            closed_width=0.0,
        )
    ...
    def joints_to_width(self, joint_positions: Sequence[float]) -> float:
        ...
        Args:
            joint_positions: The values for joints ["gripper_r_joint", "gripper_r_joint_m"].

        Returns:
            The width of the gripper corresponding to those joint positions.
        """
        # comment this out 
        #print("Joints_positions:", joint_positions)
        #print("Joints_positions:", joint_positions[0] + joint_positions[1]) 
    ...
    def width_to_joints(self, width: float) -> np.ndarray:
        ...
        Returns:
            The values for joints ["gripper_r_joint", "gripper_r_joint_m"] giving the
...
class CortexRobot(CommandableArticulation):
    ...
    class. For instance, we can construct a robot's command API using:
        ...
        usd_path = get_assets_root_path_or_die() + "/Isaac/Robots/Yumi/yumi.usd"
        add_reference_to_stage(usd_path=usd_path, prim_path=prim_path)

        robot = CortexRobot("yumi", prim_path)
    ...
    See also the CortexYumi and CortexUr10 classes which do the same, but from derived classes.
...
class MotionCommandedRobot(CortexRobot):
    ...
    def __init__(
        ...
        if settings.smoothed_rmpflow:
            ...
            # RmpFlow.set_ignore_state_updates(self, True)
        ...
        if self.settings.active_commander:
            ...
            #**************************Changed here
            articulation_motion_policy1 = ArticulationMotionPolicy(
                robot_articulation=self, motion_policy=self.motion_policy, default_physics_dt=self.commanders_step_dt
            )
            target_prim1 = VisualCuboid("/World/motion_commander_target1", size=0.01, color=np.array([0.15, 0.15, 0.15]))
            self.arm_commander1 = MotionCommander(
                articulation_motion_policy1, target_prim1, use_smoothed_commands=self.settings.smoothed_commands
            )
            #**************************
        else:
        ...
        # self.add_commander("arm1", self.arm_commander1) #Changed Here
...
class CortexYumi(MotionCommandedRobot):
    """ The Cortex Yumi contains commanders for commanding the end-effector (a MotionCommander
    governing the full arm) and the gripper (a YumiGripper governing the fingers).

    Each of these commanders are accessible via members arm and gripper.

    This object only wraps an existing USD Yumi on the stage at the specified prim_path. To
    add it to the stage first then wrap it, use the add_yumi_to_stage() method.

    Note that position and orientation are both relative to the prim the Yumi sits on.

    Args:
        name: A name for the Yumi robot. Robots added to the CortexWorld should all have unique names.
        prim_path: The path to the Yumi prim in the USD stage.
        position: The position of the robot. See CortexRobot's position parameter for details.
        orientation: The orientation of the robot. See CortexRobot's orientation parameter for details.
    ...
    def __init__(
        ...
        motion_policy_config = icl.load_supported_motion_policy_config("Yumi", "RMPflowCortex")
        super().__init__(
            name=name,
            prim_path=prim_path,
        ...
        self.gripper_commander = YumiGripper(self)
    ...
    def initialize(self, physics_sim_view: omni.physics.tensors.SimulationView = None) -> None:
        ...
        kps = np.array([6000000.0, 6000000.0, 6000000.0, 6000000.0, 2500000.0, 1500000.0, 500000.0, 6000.0, 6000.0, 6000000.0, 6000000.0, 6000000.0, 6000000.0, 2500000.0, 1500000.0, 500000.0, 6000.0, 6000.0])
        kds = np.array([300000.0, 300000.0, 300000.0, 300000.0, 90000.0, 90000.0, 90000.0, 1000.0, 1000.0, 300000.0, 300000.0, 300000.0, 300000.0, 90000.0, 90000.0, 90000.0, 1000.0, 1000.0])
        if verbose:
            print("setting yumi gains:")
            print("- kps: {}".format(kps))
            print("- kds: {}".format(kds))
        self.get_articulation_controller().set_gains(kps, kds)


def add_yumi_to_stage(
    name: str,
    prim_path: str,
    ...
    """ Adds a Yumi to the stage at the specified prim_path, then wrap it as a CortexYumi object.

    Args:
        For name, prim_path, position, orientation, and motion_commander, see the CortexYumi doc
        string.

        usd_path: An optional path to the Yumi USD asset to add. If a specific path is not
            provided, a default Yumi USD path is used.

    Returns: The constructed CortexYumi object.
    """
    if usd_path is not None:
        add_reference_to_stage(usd_path=usd_path, prim_path=prim_path)
    else:
        mg_extension_path = get_extension_path_from_name("omni.isaac.motion_generation") # changed the extension dir
        kinematics_config_dir = os.path.join(mg_extension_path, "motion_policy_configs")  # changed the sub root dir
        usd_path = kinematics_config_dir + "/yumi_description/urdf/yumi_gripper/yumi_gripper.usd" # changed the usd file location here
        
        # usd_path = get_assets_root_path_or_die() + "/Isaac/Robots/Yumi/yumi.usd"
        add_reference_to_stage(usd_path=usd_path, prim_path=prim_path)

    return CortexYumi(name, prim_path, position, orientation, use_motion_commander)
